...
def validPeriod(period):
	...
	if period not in periods:
		print ("Period Error: Period must be one of the following: %s." % (str([300, 900, 1800, 7200, 14400, 86400]).strip('[]')))
		return False
	return True
...
def buildURL(pair, period, start, end):
	...
	return '%s%s_%s&start=%s&end=%s&period=%s' % (base, pair[0], pair[1], toUnix(start), toUnix(end), period)
